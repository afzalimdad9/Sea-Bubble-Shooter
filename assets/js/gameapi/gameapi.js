...
fg_api.prototype.iapModule = function() {
	...
	module.prototype.consumeProduct = function(sku) {
		...
		return new Promise(resolve => {
			...
			if(!M.isInitialized) {
				M.log && console.warn("[IAP] Consuming product... Aborted! [Not initialized yet]");
				return resolve(result);
			}

			if(typeof sku !== "string") {
				M.log && console.warn("[IAP] Consuming product... Aborted! [Invalid param 'sku']");
				return resolve(result);
			}

			if(!(M.gameId in M.products && M.products[M.gameId].some(product => product.sku === sku))) {
				M.log && console.warn("[IAP] Consuming product... Aborted! [SKU '%s' not found]", sku);
				return resolve(result);
			}
